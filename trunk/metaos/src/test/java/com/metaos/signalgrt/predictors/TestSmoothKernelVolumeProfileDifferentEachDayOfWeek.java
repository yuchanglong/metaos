...
/**
 * Testing SmoothKernelVolumeProfileDifferentEachDayOfWeek.
 */
public class TestSmoothKernelVolumeProfileDifferentEachDayOfWeek {
    ...
    public void testSimulatedLearning() throws Exception {
        ...
        final PredictorListener predictor = 
                new SmoothKMAVolumeProfileDifferentEachDayOfWeek(
                        new Predictor.PredictorSelectionStrategy() {
                            public void injectKernel(final Predictor p) {
                                ((KernelMovingWindow)p).setKernel(
                                    new KernelMovingWindow.Kernel() {
                                    public int getKernelSize() { 
                                        return 3; // Three weeks 
                                    }
                                    public double eval(double[] memory, 
                                            int headIndex) {
                                        double total = 0;
                                        for(int i=0; i<3; i++) {  // Three weeks
                                            int index = (i + headIndex) % 3;
                                            total += memory[index];
                                        }
                                        return total / 3; // Three weeks
                                    }
                                });
                            }
                            ...
                            public PredictorListener buildPredictor() {
                                return new PredictorListener.SingleSymbolField(
                                        new KernelMovingWindow(3), symbol, 
                                                new Field.VOLUME());
                            }
...
*/
    ...
    public static void main(final String args[]) throws Exception {
        try {
            new Engine();
            final TestSmoothKernelVolumeProfileDifferentEachDayOfWeek test =
                    new TestSmoothKernelVolumeProfileDifferentEachDayOfWeek();

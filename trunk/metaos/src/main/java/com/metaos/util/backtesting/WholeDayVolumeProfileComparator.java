...
public class WholeDayVolumeProfileComparator 
    ...
    public WholeDayVolumeProfileComparator(
        ...
        this(instantGenerator, field, minimumDay, 1);
    }
    ...
    public WholeDayVolumeProfileComparator(
            final CalUtils.InstantGenerator instantGenerator,
            final Field field, final Calendar minimumDay, final int groupSize) {
        super(instantGenerator, field, minimumDay);
    ...
    protected double[] contrast(final double a[], final double b[]) {
        ...
        for(int i=0; i<a.length; i+=this.groupSize) {
            for(int j=0; j<this.groupSize; j++) {
                sumA += a[i+j]>=0 ? a[i+j] : 0;
                sumB += b[i+j]>=0 ? b[i+j] : 0;
            }
        }
        ...
        for(int i=0; i<a.length; i+=this.groupSize) {
            ...
            if(a2>0 && b2>0) {
                double e = (a2 / sumA) - (b2 / sumB);
                diffs[i/this.groupSize] = (e*e) / b2;
            } else {
